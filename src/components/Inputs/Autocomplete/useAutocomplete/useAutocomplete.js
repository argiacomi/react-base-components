...
export default function useAutocomplete(props) {
  const {
    isActiveElementInListbox = defaultIsActiveElementInListbox,
    autoComplete = false,
    autoHighlight = false,
    autoSelect = false,
    blurOnSelect = false,
  ...
  if (!import.meta.env.PROD) {
    if (value !== null && !freeSolo && options.length > 0) {
      ...
      if (missingValue.length > 0) {
        console.warn(
          [
            `The value provided to ${componentName} is invalid.`,
    ...
    React.useEffect(() => {
      if (!inputRef.current || inputRef.current.nodeName !== 'INPUT') {
        if (inputRef.current && inputRef.current.nodeName === 'TEXTAREA') {
          ...
          console.error(
            [
              `Unable to find the input element. It was resolved to ${inputRef.current} while an HTMLInputElement was expected.`,
  ...
  const handleBlur = (event) => {
    if (isActiveElementInListbox(listboxRef)) {
      inputRef.current.focus();
      return;
    }
