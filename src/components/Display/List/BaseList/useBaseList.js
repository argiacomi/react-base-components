...
function useBaseList(params) {
  ...
  if (!import.meta.env.PROD) {
    if (focusManagement === 'DOM' && getItemDomElement == null) {
      throw new Error(
        'useBaseList: The `getItemDomElement` prop is required when using the `DOM` focus management.'
      );
    }

    if (focusManagement === 'activeDescendant' && getItemId == null) {
      throw new Error(
        'useBaseList: The `getItemId` prop is required when using the `activeDescendant` focus management.'
      );
    }
  }
  ...
  const reducer = externalReducer ?? baseListReducer;
